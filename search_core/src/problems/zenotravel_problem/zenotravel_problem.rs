...
/// Represents the state extracted from a PDDL problem.
///
/// The state includes:
/// - The number of cities in the problem.
/// - A map of airplanes where the key is the airplane index and the value is an `Airplane` struct.
/// - A map of distances between pairs of cities.
/// - Global metrics such as total fuel used and total time.
...
impl Problem for ZenoTravelProblem {
    ...
    pub fn load_state_from_json(json_path: &str) -> (State, Self) {
        ...
        let goal_obj = goal_value.as_object().expect("Goal field must be an object");
        ...
        if let Some(distances_json) = json_obj.get("distances").and_then(|v| v.as_object()) {
            for (key, value) in distances_json {
                ...
                if parts.len() == 2 {
                    ...
                    let distance = value
                        .as_i64()
                        .expect("Invalid distance value") as i32;
