    ...
    pub fn place_object(&mut self, object: &Vehicle) -> Result<String, String> {
        ...
        for &[x, y] in &positions {
            if x >= self.row_size || y >= self.col_size {
                return Err(format!("Object {} is out of bounds at ({}, {}).", object.name, x, y));
            }
        ...
        Ok(format!("{} placed successfully at {:?}", object.name, positions))
    }
...
impl RedCarProblem {
    ...
    fn apply_move_action(state: &State, action: &Action, direction: &str) -> State {
        ...
        let vehicle_index = new_state.vehicle_state.vehicles.iter().position(|v| v.name == vehicle_name);
        ...
        if let Some(index) = vehicle_index {
            ...
            // Update the grid
            new_state.grid.update(old_positions, new_positions, vehicle.name.clone());
        } else {
...
impl Problem for RedCarProblem {
    ...
    fn heuristic(&self, state: &State) -> f64 {
        ...
        fn load_state_from_json(json_path: &str) -> (State, Self) {
            ...
            let (problem_name, problem_data) = json_data.as_object()
            ...
            if let Some(vehicles) = problem_data["vehicles"].as_array() {
                for v in vehicles {
                    ...
                    grid.place_object(&vehicle).expect("Failed to place vehicle on grid");
                }
            }
