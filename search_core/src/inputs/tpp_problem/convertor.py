...
def convert_location(loc_str):
    """Converts a location string to a string value:
       ...
       - If it starts with 'market', returns (market number - 1) as a string.
...
def parse_truck_locations(init_section):
    """Parses all (loc truck depot) statements and returns a dict mapping truck name to its location (as a string)."""
    truck_locations = {}
...
def parse_market_items(init_section):
    """Parses price and on-sale info for markets.
       ...
       Each item is stored as: { goods_id: { "price": <float>, "on_sale": <int> } }
    """
    market_items = {}
...
def parse_pddl(pddl_text):
    ...
    for market in objects_by_type.get("market", []):
        state_markets.append({
            "location": convert_location(market),
    ...
    for from_loc, to_loc, cost in drive_cost_pattern.findall(init_section):
        cost = float(cost)
        conv_from = convert_location(from_loc)
        conv_to = convert_location(to_loc)
        # Create the key in the format "(from,to)".
